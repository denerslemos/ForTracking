...
// skip tracks that fail cuts, using vertex with most tracks as PV       
     ...
     ptreco_->Fill(tr->pt());
     etareco_->Fill(tr->eta());
     phireco_->Fill(tr->phi());
     ...
     if(recSimColl.find(track) != recSimColl.end())
     {
       ...
       if( mtp->status() < 0 ) 
       {
         ...
         pttrue_->Fill(tr->pt());
     	 etatrue_->Fill(tr->eta());
     	 phitrue_->Fill(tr->phi()); 
       }
     }
     else
     {
       ...
       ptfake_->Fill(tr->pt());
       etafake_->Fill(tr->eta());
       phifake_->Fill(tr->phi()); 
   ...
   {      
     ...
     if(tp->status() < 0) continue; //only charged primaries
